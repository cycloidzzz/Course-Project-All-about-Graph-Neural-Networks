...
def get_adj(adj):
    ...
    '''
    d_rev = np.power(rowsum, -1).flatten()
    d_rev[np.isinf(d_rev)] = 0
    d_mat = sp.diags(d_rev)
...
def normalized_adj(adj):
    ...
    print("order 1 adj size is", adj1.size)
    adj2 = adj1.dot(adj1)
    print("order 2 adj size is", adj2.size)
    adj3 = adj2.dot(adj1)
    norm1 = get_adj(adj1)
    norm2 = get_adj(adj2)
    return sparse_to_tuple(norm1), \
           sparse_to_tuple(norm2)
